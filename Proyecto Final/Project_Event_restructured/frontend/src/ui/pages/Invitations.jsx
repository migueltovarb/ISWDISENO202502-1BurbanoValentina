...
export default function Invitations(){
  ...
  const [form,setForm] = useState({ code:'', eventId:'' })
  const [err,setErr] = useState(null)

  const load = async () => {
    try{ const {data} = await api.get('/api/invitation-codes'); setList(data?.items || data || []) }catch(e){ setErr(e?.response?.data || e.message) }
  }
  useEffect(()=>{ load() }, [])

  const create = async (e) => {
    e.preventDefault()
    try{ await api.post('/api/invitation-codes', form); setForm({ code:'', eventId:''}); load() }catch(e){ setErr(e?.response?.data || e.message) }
  }

  return (
    <div className="grid md:grid-cols-2 gap-6">
      <div className="card">
        <h2 className="text-xl font-semibold mb-3">Crear Código</h2>
      ...
      <div className="card">
        <h2 className="text-xl font-semibold mb-3">Códigos</h2>
        <ul className="space-y-2">
          {list.map((c,idx)=>(
            <li key={idx} className="p-3 rounded-xl border flex items-center justify-between">
              <div>{c.code}</div>
              <div className="text-sm text-neutral-500">Evento: {c.eventId}</div>
            </li>
          ))}
        </ul>
      </div>
